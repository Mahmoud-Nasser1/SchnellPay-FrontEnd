import { Edit2, Trash2, Layers, Search, Loader2, ChevronLeft, ChevronRight } from "lucide-react";
              ...
              {loading ? (
                ...
                services.map((s, i) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-muted-foreground">{s.provider_name}</td>
                    <td className="px-5 py-3 font-mono text-foreground">{parseFloat(s.fee).toFixed(2)}%</td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <Switch checked={s.is_active} onCheckedChange={() => onToggle(s)} />
                        <span
                          className={cn(
                            "text-xs font-semibold",
                            s.is_active ? "text-accent" : "text-muted-foreground",
                          )}
                        >
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="inline-flex gap-1">
                        <Button variant="ghost-muted" size="icon-sm" onClick={() => onEdit(s)}>
        ...
        {total > 0 && (
          <div className="flex items-center justify-between border-t border-border px-5 py-4 bg-muted/20">
            <p className="text-xs text-muted-foreground">
              ...
              <span className="font-medium text-foreground">{total}</span> services
            </p>
